...
/*
    // 사진 크게보기 - 기존 2022.09.16   
    var photo_pop_img_now_cnt = 0
    $('.photo_pop_img').on('load', function() { 
        // do stuff
    }).each(function() {
        if (this.complete) {
...
*/
    ...
    $( document ).ready( function() {
        $('#photo_pic_pop_wrapper').imagesLoaded()
            .progress( function( instance, image ) {
                ...
                if (image.img.width  > nw) {
                    step1_w = nw - photo_pop_img_lr_margin
                    step1_h = Math.floor(image.img.height * (step1_w) / image.img.width)
                    
                    $( image.img ).width((step1_w))
                    $( image.img ).height(step1_h)
                    // console.log('view_w:' + nw + ' view_h:' + nh + ' ori_w:' + image.img.width + ' ori_h:' + image.img.height + ' step1_w:' + step1_w + ' step1_h:' + step1_h)            
    
                }

                if (step1_h > nh) {
                    step2_h = nh - photo_pop_img_top_margin
                    step2_w = Math.floor(step1_w * (step2_h) / step1_h)
    
                    $( image.img ).height(step2_h)
                    $( image.img ).width(step2_w)
                }
    
                // vertical align middle
                if (step2_h == 0) step2_h = step1_h 
                var new_top = (nh - step2_h) / 2
                $parent.css('margin-top', new_top)
                $parent.css('width', nw - photo_pop_img_lr_margin)
        });
    } );
    ...
    $('.map_img_zoom_btn').on('click', function() {
        window.open($('.map_view_img').attr('src'))
    })
